...
import Versions from './components/Versions'; // Adjust the path as necessary
...
React.useEffect(() => {
  ...
  window.electron.ipcRenderer.on('window-focused', (evt, renderer: any) => {
    logger.debug(`window-focused - renderer:`, renderer)
  ...
  <>
    <div className="flex flex-col h-screen w-screen">
      <header className="h-[60px] bg-gray-800 text-white flex items-center px-6">
        <h1 className="text-xl font-bold">React Header</h1>
      </header>

      <main className="flex-1 flex">
        <div className="w-1/4 bg-gray-100 p-6">
          <h2 className="text-lg font-semibold mb-4">Left Column</h2>
      ...
      <footer className="h-[60px] bg-gray-800 text-white flex items-center px-6">
        <p>Footer from React</p>
      </footer>
    </div>
    </>
  );
